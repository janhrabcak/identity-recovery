...
function manageGitHubIssue(result) {
  ...
  if (status === 'EXPIRING_SOON' || status === 'STALE') {
    ...
    const body = [
      ...
      '',
      '### 🛡️ Why This Matters',
      'In a physical disaster scenario (lost phone, lost YubiKey, lost wallet), stale 2SV backup codes or outdated password manager emergency keys will lead to permanent lockout.',
      '',
      '### 📋 Recommended Rotation Steps:',
      '1. Generate new 2SV backup codes and verify your password manager emergency credentials.',
