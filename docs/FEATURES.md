...
## 9. 🛠️ Split-Pane Vault Studio Workspace (`tools/builder.html`)
...
- **Split-Pane Ergonomics:** Replaces vertical form clutter with a persistent left rail (vault deck overview, quick-add palette, and master key status) and an active canvas.
- **Real-Time Live Terminal Preview:** Displays an authentic, synchronized mockup of how the selected credential card appears inside the decrypted terminal ([`public/index.html`](../public/index.html)) before you encrypt.
- **Embedded Diceware Engine:** Generates high-entropy 8-word phrases using `crypto.getRandomValues` and a curated dictionary, with live entropy indicators (words, characters, unique count).
